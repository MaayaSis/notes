...
## 有 key 的 diff 算法
   ...
   1. 使用路由懒加载,将首屏渲染使用不到的组件,只在需要用到或浏览器空闲时间请求拿取
   2. 这是因为懒加载会让`webpack`对网页开发者自己编写的代码中暂时用不到的代码进行分包
   3. 一般来说分包的名字:`chunk+hash`,可以使用魔法注释自定义`webpack`分包名称
   4. 魔法注释:`component:() => impport(/* webpackChunkName: “home-chunk” */“../pages/Home”)`
   ...
   <!-- app.vue --> 
   <template>
   	<router-link :to="/user/MaayaSis/id/53"></router-link>
   <template>
   ```
   ...
   const routes = [
     ...
     // 当上面的所有路由都未能匹配到当前 url 路径时,就会自动匹配最后一个通配符的路由路径
     {
       path: '*' // path: '/:pathMatch(.*)'
       name: 'user',
   ...
   // 当用户输入路径 localhost:8080/uufdasf/fasdfasdfasdf/fasaf 进入 NotFound 版面后
   // NotFound.vue
   const pathMatch = this.$route.params.pathMatch // 取得uufdasf/fasdfasdfasdf/fasaf
   
   
   // 如果改为 path: '/:pathMatch(.*)*'
   const pathMatch = this.$route.params.pathMatch // 取得 ['uufdasf', 'fasdfasdfasdf', 'fasaf']
   ```
   ...
   <script>
     export default {
       ...
       setup() {
         const router = useRouter // 因为 vue3 中的 setup 无法取到 this
         router.push('/user/MaayaSis/id/53')
         router.push({ // 执行后页面的 url: localhost:8080/user/MaayaSis/id/53?age=18
           path: '/user/MaayaSis/id/53',
           query: {
             agel: 18
           }
         })
